...
{
public:
    SpawnerNode() : Node("turtle_spawner"), turtle_counter_(0)
    {
        ...
        alive_turtles_publisher_ = this->create_publisher<turtle_interfaces::msg::TurtleArray>("alive_turtles", 10);
        
        publish_alive_turtles_timer_ = this->create_wall_timer(std::chrono::seconds(1), std::bind(&SpawnerNode::publishAliveTurtles, this));
        
        spawn_turtle_timer_ = this->create_wall_timer(std::chrono::milliseconds((int)(1000.0 / spawn_frequency_)), std::bind(&SpawnerNode::spawnNewTurtle, this));
        
        catch_turtle_service_ = this->create_service<turtle_interfaces::srv::CatchTurtle>("catch_turtle", std::bind(&SpawnerNode::callbackCatchTurtle, this, std::placeholders::_1, std::placeholders::_2));
    }
...
private:
    ...
    {
        kill_turtle_threads_.push_back(std::make_shared<std::thread>(std::bind(&SpawnerNode::callKillTurtleService, this, request->name)));
    ...
    void spawnNewTurtle()
    {
        ...
        spawn_turtle_threads_.push_back(
            std::make_shared<std::thread>(
                std::bind(&SpawnerNode::callSpawnTurtleService, this, x, y, theta)));
    }
    ...
    rclcpp::Publisher<turtle_interfaces::msg::TurtleArray>::SharedPtr alive_turtles_publisher_;
